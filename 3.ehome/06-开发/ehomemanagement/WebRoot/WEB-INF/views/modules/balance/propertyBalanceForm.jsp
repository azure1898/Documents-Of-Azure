...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/balance/propertyBalance/form?id=${propertyBalance.id}">物业结算信息<shiro:hasPermission name="balance:propertyBalance:edit">${not empty propertyBalance.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="balance:propertyBalance:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="propertyBalance" action="${ctx}/balance/propertyBalance/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="balanceStartTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${propertyBalance.balanceStartTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="balanceEndTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${propertyBalance.balanceEndTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="balance:propertyBalance:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
