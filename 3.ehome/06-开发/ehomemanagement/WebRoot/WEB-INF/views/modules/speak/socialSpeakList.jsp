...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/speak/socialSpeak/">发言管理</a></li>
	</ul>
		...
		<ul class="ul-form">
			...
			<li class="btns">
				<form:select path="delflag" class="input-small">
					...
					<form:options items="${fns:getDictList('social_delflag')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><span>&nbsp;</span>
	...
	<ul style="margin:10px;">
		<li style="list-style-type:none;">
			<shiro:hasPermission name="village:villageInfo:edit">
				<a id="btuElemAdd" href="${ctx}/speak/socialSpeak/form?id=" class="btn btn-primary"><i class="icon-plus icon-custom"></i>添加</a>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="socialSpeak" varStatus="status">
			<tr onClick="selectElem(this)">
				...
				<td>
					${socialSpeak.villageinfoname}
				</td>
				<td>
					<c:if test="${socialSpeak.istop =='1'}">
						<font color="red">【置顶】</font>
					</c:if>
					<c:if test="${socialSpeak.subjectname != '' && socialSpeak.subjectname != null}">
						<font color="blue">#${socialSpeak.subjectname}#</font>
					</c:if>
				...
				<td>
					...
					<%-- <img id="preview" src="${socialSpeak.images}" style="width:45px; height:45;" /> --%>
				</td>
				<td>
					${fns:getDictLabel(socialSpeak.visrange, 'visRange', '')}
				</td>
				<td>
					${socialSpeak.readnum}/${socialSpeak.sumcomment}/${socialSpeak.sumforward}
				</td>
				<td>
					
				</td>
				<td>
					${socialSpeak.username}
				</td>
				<td>
					${fns:getDictLabel(socialSpeak.delflag, 'social_delflag', '')}
