 ...
 */
package com.its.modules.sys.utils;
...
import com.its.modules.sys.dao.MenuDao;
...
import com.its.modules.sys.security.SystemAuthorizingRealm.Principal;
...
public class UserUtils {
    ...
    public static User getByLoginName2(String loginName) {
        User user = new User();
        user = userDao.getByLoginName2(new User(null, loginName));
        return user;
    }
     ...
     * @return List<Role>
     * @author zhujiao
     * @date 2017年7月19日 下午12:07:59
     */
    public static List<Role> findAllListM(Role role) {
        @SuppressWarnings("unchecked")
        List<Role> roleList = (List<Role>) getCache(CACHE_ROLE_LIST);
        User user = getUser();
        if (user.isAdmin()) {
            roleList = roleDao.findAllListM(role);
        } else {
            Role role1 = new Role();
            role1.getSqlMap().put("dsf", BaseService.dataScopeFilter(user.getCurrentUser(), "o", "u"));
            roleList = roleDao.findList(role1);
        }
        return roleList;
    }
