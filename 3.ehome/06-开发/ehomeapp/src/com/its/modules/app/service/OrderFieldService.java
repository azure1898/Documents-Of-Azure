...
public class OrderFieldService extends CrudService<OrderFieldDao, OrderField> {
	...
	@Transactional(readOnly = false)
	public void update(OrderField orderField) {
		dao.update(orderField);
	}
	 ...
	 * @return OrderField
	 */
	public OrderField judgeOrderFieldByOrderIdAndAccountId(String orderId, String accountId) {
		return dao.judgeOrderFieldByOrderIdAndAccountId(orderId, accountId);
	}
	...
	public boolean cancelOrder(String orderId, String accountId) {
		// 判断订单是否可取消
		OrderField orderField = this.judgeOrderFieldByOrderIdAndAccountId(orderId, accountId);
		if (orderField == null) {
			return false;
		}

		// 待支付——取消订单——已取消
		orderField.setOrderState(OrderGlobal.ORDER_GOODS_CANCELED);
