...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
		...
		<input id="selectElemId" type="hidden" value=""/><%-- add--%>
		...
		<ul style="margin:10px;">
		     <li style="list-style-type:none;">
		     	<shiro:hasPermission name="sys:user:edit">
					<a href="${ctx}/sys/user/form?id=" class="btn btn-primary"><i class="icon-plus icon-custom"></i> 添加</a><%-- add--%>
	...
	<table id="contentTable" class="table table-bordered table-condensed"> <%-- add--%>
		<thead>
			<tr align="center">
...
<%-- 				</shiro:hasPermission> --%>
		...
		<tbody>
			<c:forEach items="${page.list}" var="user" varStatus="status">
				<tr onClick="selectElem(this)"> <%-- add--%>
					<td>
						${status.index+1}
						<input id="oldFrozen" type="hidden" value="${user.loginFlag}" />
