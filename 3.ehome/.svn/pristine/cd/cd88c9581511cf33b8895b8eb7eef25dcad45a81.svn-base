...
public class Order {
	...
	public void setType(int type) {
		if (type == 0) {// 商品库存不足提醒
			this.url = "/goods/goodsInfo?id=" + id;
		} else if (type == 1) {// 服务库存不足提醒
			this.url = "/service/serviceInfo?id=" + id;
		} else if (type == 2) {// 课程预约预警提醒
			this.url = "/lesson/lessonInfo?id=" + id;
		} else if (type == 3) {// 场地预约预警提醒
			this.url = "/field/fieldInfo?id=" + id;
		} else if (type == 10) {// 商品订单
			this.url = "/order/orderGoods?orderNo=" + orderNo;
		} else if (type == 11) {// 服务订单
			this.url = "/order/orderService?orderNo=" + orderNo;
		}
		this.type = type;
	}
	...
	public String toString() {
		String title = "", ptitle = "", content = "", sucbtn = "", celbtn = "返回", show = "true", buttons = "";
		if (type == 0) {// 商品库存不足提醒
			title = "您有商品即将售空:";
			content += "商品名称:" + name + "<br />";
			...
			title = "您有服务即将售空:";
			content += "服务名称:" + name + "<br />";
			content += "库存:" + stock;
			sucbtn = "进入服务列表";
			ptitle = "服务管理";
		} else if (type == 2) {// 课程预约预警提醒
			title = "课程预约提醒:";
			content += name + " 限制人数即将约满";
			sucbtn = "进入课程预约列表";
			ptitle = "课程预约管理";
		} else if (type == 3) {// 场地预约预警提醒
			title = "场地预约提醒:";
