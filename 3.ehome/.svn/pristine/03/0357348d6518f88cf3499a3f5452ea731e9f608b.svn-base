...
<head>
	...
	<style>
		.addPriceSpan{
			border: solid thin #797979;font-size: 25px; vertical-align: middle;border-radius:5px;margin: 1px 0 0 30px;cursor: pointer;
	...
	<script type="text/javascript">
		...
		function addFieldPrice(e) {
			...
			var html="<div name=\"fdxx\" id=\""+index_num+"\" class=\"controls\" style=\"border: solid thin #fff;margin: 15px 20px 15px 160px;padding-left: 15px;padding-bottom: 15px;  -moz-border-radius: 10px;-webkit-border-radius: 10px; border-radius: 10px;\">\n" +
                "\t\t\t\t<br>\n" +
                "\t\t\t\t<div>\n" +
                ...
                "\t\t\t\t\t</span>\n" +
				"\t<input type=\"text\" readonly=\"readonly\" style=\"box-shadow:none; border: 0px solid #f2f2f2; background-color: #f2f2f2;width:0px;height:0px;\" id=\"strRequired"+index_num+"\"  name=\"strRequired"+index_num+"\" value=\"\" class=\"input-xlarge required\">" +
                "\t\t\t\t</div>\n" +
                "\t\t\t\t<br>\n" +
                "\t\t\t\t时段：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n" +
                "\t\t\t\t<select id=\"fieldInfoPrices"+index_num+"_startOpenTime\" style=\"width:150px;\" name=\"fieldInfoPrices["+index_num+"].startOpenTime\" class=\"input-xlarge required\">\n" +
                "\t\t\t\t\t<option value=\"\"></option>\n";
			//生成分段价格 预约时段
            ...
            for (var j=minHour;j<=maxHour;j++){
                ...
                html+="\t\t\t\t</select>\n" +
                "\t\t\t\t至\n" +
                "\t\t\t\t<select id=\"fieldInfoPrices"+index_num+"_endOpenTime\" style=\"width:150px;\" name=\"fieldInfoPrices["+index_num+"].endOpenTime\" class=\"input-xlarge required\">\n" +
                "\n" +
                "\t\t\t\t</select>\n" +
    	...
    	function onChangeShortTime() {
    		$("#shortTime").removeClass('error');
        	$("#shortTime").addClass('valid');
        	$("#shortTime").next('label').attr('style','display:none;')
    ...
    $(document).ready(function() {
        ...
        $("select[name=startOpenTime]").live("change",function () {
        	...
        	$("select[name=startOpenTime]").removeClass('error');
        	$("select[name=startOpenTime]").addClass('valid');
        	$("select[name=startOpenTime]").next('label').attr('style','display:none;')
        ...
        $("select[name=endOpenTime]").live("change",function () {
        	...
        	$("select[name=endOpenTime]").removeClass('error');
        	$("select[name=endOpenTime]").addClass('valid');
        	$("select[name=endOpenTime]").next('label').attr('style','display:none;')
        ...
        $("input[name=isChild]").live("change",function () {
            ...
            if($(this).attr('checked')=="checked"||$(this).attr('checked')=="true"){
                ...
                if(checkfd()){
                	...
                	bd="您还有分时段价格规则未填写，取消勾选将会删除该条分时段价格规则.";
                }
                top.$.jBox.confirm(bd, "提示",function (v, h, f) {
                    if(v == 'ok'){
                        $("#isAll").remove();
                        $("#delId").remove();
...
*/
		...
		function fieldSubmit() {
			$("#createState").val("1");
			//判断一下是否有分段信息
			if(!$("input[name=isChild]").is(":checked")){
				$("#fstate").val("0");
			}
			$("#inputForm").submit();
        }
        ...
        function checkfd(){
        	var _b = true;
        	$('div[name=fdxx]').each(function(k,v){
        		//验证日期是否已勾选
        		var _c = false;
        		...
        		if(!_c){_b=false;return false;}
        		//验证时段开始是否已设置
        		if($(this).find('select[id$=startOpenTime]').val()==""){
        			_b=false;
        			return false;
        		}
        		//验证时段结束是否已设置
        		if($(this).find('select[id$=endOpenTime]').val()==""){
        			_b=false;
        			return false;
        		}
        		//验证价格是否已设置
        		if($(this).find('input[name$=basePrice]').val()==""){
        			_b=false;
        			return false;
        		}
        	});
	...
	<style type="text/css">
		...
		.weeks span:nth-child(4){
			margin-right:500px;
			margin-top: 30px;
		}
		.weeks span:nth-child(7){
			margin-right:0px;
			margin-top: 30px;
		}
	</style>
...
<body>
	<ul class="nav nav-tabs">
		...
		<input type="hidden" value="1" name="fstate" id="fstate">
		<sys:message content="${message}"/>

		<div class="form-group">
			<label class="col-lg-2 control-label" style="text-align: left;">场地名称：</label>
			<div class="col-lg-10">
				<form:input path="name" htmlEscape="false" maxlength="10"
					style="width: 305px;" class="input-xlarge required fieldname" />
				<span class="help-inline">(最多不超过10个字) </span>
			</div>
		</div>
		<div class="form-group" style="height: 38px;">
		    <label class="col-lg-2 control-label" style="border-right: 2px solid #FFF;text-align: left;">
				营业时间：<br><input type="checkbox" onclick="checkWeekAll(this)" ><label style="font-size: 14px;">全选</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			</label>
		    <div class="col-lg-10 weeks">
			    <form:checkboxes path="weekStr" cssStyle=""  items="${fns:getDictList('week')}" itemLabel="label" itemValue="value" htmlEscape="false" class="input-xlarge required"/>
		    </div>
		  </div>
		  
   			<div class="form-group">
			    <label class="col-lg-2 control-label" style="text-align: left;">最短预约时间：</label>
			    <div class="col-lg-10">
				    <form:select path="shortTime" onchange="onChangeShortTime()" cssStyle="width:320px;" class="input-xlarge required">
		...
		<div class="form-group">
		    <label class="col-lg-2 control-label" style="text-align: left;">开放预约时段：</label>
		    <div class="col-lg-10">
			    <select id="startOpenTime" style="width: 150px;" name="startOpenTime"  class="input-xlarge required">
					<option value=""></option>
				...
				至
				<select name="endOpenTime" style="width: 150px;" id="endOpenTime" class="input-xlarge required">

				</select>
				<span class="help-inline">(请选择最短预约时间整倍数时间段作为营业时间段)</span>
		    </div>
		  </div>
		  <div class="form-group">
	    	...
	    	<div class="col-lg-10">
	    		<form:checkboxes path="isChild" items="${fns:getDictList('checkboxs_one')}" itemLabel="label" itemValue="value" htmlEscape="false" class=""/>分时段价格
	    	</div>
		    
		    
		    <div id="files_price" class="control-group">
			<c:forEach items="${fieldInfoPrices}" var="fieldInfoPrice" varStatus="sta">
				<div id="${sta.index}" class="controls" style="border: solid thin #fff;margin: 15px 20px 15px 160px;padding-left: 15px;padding-bottom: 15px;  -moz-border-radius: 10px;-webkit-border-radius: 10px; border-radius: 10px;">
					...
					<span style="margin-right:90px;">
						...
						<input type="text" value="1" id="strRequired${sta.index}" name="strRequired${sta.index}" style="box-shadow:none; border: 0px solid #f2f2f2; background-color: #f2f2f2;width:0px;height:0px;" class="input-xlarge required">
					</div>
					<br>
					时段：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<select id="fieldInfoPrices${sta.index}_startOpenTime" style="width:150px;" name="fieldInfoPrices[${sta.index}].startOpenTime" class="input-xlarge required">
						<option value=""></option>
					...
					至
					<select id="fieldInfoPrices${sta.index}_endOpenTime" style="width:150px;" name="fieldInfoPrices[${sta.index}].endOpenTime" class="input-xlarge required ">
		...
		<div style=" width: 800px;margin: 10px auto;text-align: right;">
			...
			<c:if test="${tag!=0 }">
				<input id="btnSubmit" class="commonsmallbtn" style="width: 50px;height: 30px;font-size: 14px;" type="submit" value="保存"/>&nbsp;
			</c:if>
			</shiro:hasPermission>
			<shiro:hasPermission name="field:fieldInfo:edit">
				<input id="btnSubmit" class="commonsmallbtn" type="button" style="width: 120px;height: 30px;font-size: 14px;" onclick="fieldSubmit()" value="保存并生成预约"/>&nbsp;
			</shiro:hasPermission>
			<input id="btnCancel" class="commonbtn" type="button" value="返 回" style="width: 50px;height: 30px;border-radius:10px;font-size: 14px;" onclick="window.location.href = '${ctx}/field/fieldInfo/list'"/>
		</div>
	</form:form>
</body>
</html>