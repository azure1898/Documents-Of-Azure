...
<head>
    ...
    <script type="text/javascript">
        ...
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            if ($("#endCreateDate").val() != null && $("#endCreateDate").val() != "" 
                    && $("#beginCreateDate").val() != null && $("#beginCreateDate").val() != "" 
                    && $("#endCreateDate").val() < $("#beginCreateDate").val()) {
...
<body>
    ...
    <form:form id="searchForm" modelAttribute="noticeManage" action="${ctx}/notice/noticeManage/" method="post" class="breadcrumb form-search">
        ...
        <ul class="ul-form">
            <li>
                <input id="beginCreateDate" name="beginCreateDate" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate"
                    value="<fmt:formatDate value="${noticeManage.beginCreateDate}" pattern="yyyy-MM-dd"/>"
                    onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> - 
                <input id="endCreateDate" name="endCreateDate" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate"
                    value="<fmt:formatDate value="${noticeManage.endCreateDate}" pattern="yyyy-MM-dd"/>"
                    onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
            </li>
            <li class="btns">
    ...
    <table id="contentTable" class="table table-bordered table-condensed">
        ...
        <c:forEach items="${page.list}" var="noticeManage" varStatus="status">
            <tr onClick="selectElem(this)">
                ...
                <td>
                    <fmt:formatDate value="${noticeManage.createDate}" pattern="yyyy-MM-dd"/>
                </td>
            </tr>
        </c:forEach>
        </tbody>
