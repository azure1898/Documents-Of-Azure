...
$(function() {
	...
	$(".file").change(function() {
		...
		if (fileList.length > 9 || totalNum > 9) {
			alertx("上传图片数目不可以超过9个，请重新选择"); // 一次选择上传超过9个
		...
		tempInterval = self.setInterval(function() {
			temp(fileList, tempInterval, arrImgs, numUp, imgContainer)
		}, 200);
	});
	/* 点击图片的文本框 */
		...
		if (flag) {
			if (fileList.length = 0) {
				...
				for (var i = 0; i < fileList.length; i++) {
					...
					$img0.attr("src", "/ehomemanagement/static/images/a7.png")
							.appendTo($section);
					...
					var $input3 = $("<input name='picList[" + img_index
							+ "].type' value='" + fileList[i].type
							+ "' type='hidden'/>");
					...
					reader.onload = function(e) {
						if (document.getElementsByName("picList[0].imgBase64").length != 0
								&& document
							...
							document.getElementsByName("imgName4")[0].src = this.result;
								...
								.getElementsByName("picList[5].imgBase64").length != 0
								&& document
										.getElementsByName("picList[5].imgBase64")[0].innerHTML == "") {
							document.getElementsByName("picList[5].imgBase64")[0].innerHTML = this.result;
							document.getElementsByName("imgName5")[0].src = this.result;
						} else if (document
								.getElementsByName("picList[6].imgBase64").length != 0
								&& document
										.getElementsByName("picList[6].imgBase64")[0].innerHTML == "") {
							document.getElementsByName("picList[6].imgBase64")[0].innerHTML = this.result;
							document.getElementsByName("imgName6")[0].src = this.result;
						} else if (document
								.getElementsByName("picList[7].imgBase64").length != 0
								&& document
										.getElementsByName("picList[7].imgBase64")[0].innerHTML == "") {
							document.getElementsByName("picList[7].imgBase64")[0].innerHTML = this.result;
							document.getElementsByName("imgName7")[0].src = this.result;
						} else if (document
								.getElementsByName("picList[8].imgBase64").length != 0
								&& document
										.getElementsByName("picList[8].imgBase64")[0].innerHTML == "") {
							document.getElementsByName("picList[8].imgBase64")[0].innerHTML = this.result;
							document.getElementsByName("imgName8")[0].src = this.result;
						}
					}
				}
			}
