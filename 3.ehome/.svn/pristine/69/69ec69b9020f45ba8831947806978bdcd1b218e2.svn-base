...
public class OrderLessonService extends CrudService<OrderLessonDao, OrderLesson> {
	...
	@Transactional(readOnly = false)
	public void update(OrderLesson orderLesson) {
		dao.update(orderLesson);
	}
	 ...
	 * @return OrderLesson
	 */
	public OrderLesson judgeOrderLessonByOrderIdAndAccountId(String orderId, String accountId) {
		return dao.judgeOrderLessonByOrderIdAndAccountId(orderId, accountId);
	}
	...
	public boolean cancelOrder(String orderId, String accountId) {
		// 判断订单是否可取消
		OrderLesson orderLesson = this.judgeOrderLessonByOrderIdAndAccountId(orderId, accountId);
		if (orderLesson == null) {
			return false;
		}
		// 待支付——取消订单——已取消
		orderLesson.setOrderState(OrderGlobal.ORDER_GOODS_CANCELED);
		// 更新订单主表
		this.update(orderLesson);
		// 恢复服务库存
		LessonInfo lessonInfo = lessonInfoService.get(orderLesson.getLessonInfoId());
		lessonInfo.setPeopleLimit(lessonInfo.getPeopleLimit() + 1);
		lessonInfo.setSellCount(lessonInfo.getSellCount() - 1);
		lessonInfoService.update(lessonInfo);
		// 插入订单追踪
