...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			if ($("#endRegistDate").val() != null && $("#endRegistDate").val() != "" 
				    && $("#beginRegistDate").val() != null && $("#beginRegistDate").val() != "" 
				    && $("#endRegistDate").val() < $("#beginRegistDate").val()) {
...
<body>
	...
	<form:form id="searchForm" modelAttribute="account" action="${ctx}/account/account/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
		    <li>
				<input id="beginRegistDate" name="beginRegistDate" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate"
					value="<fmt:formatDate value="${account.beginRegistDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> - 
				<input id="endRegistDate" name="endRegistDate" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate"
					value="<fmt:formatDate value="${account.endRegistDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
			</li>
			<li class="btns">
