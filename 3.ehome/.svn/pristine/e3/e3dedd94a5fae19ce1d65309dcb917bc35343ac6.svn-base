		...
		success: function(data){
			...
			$.each(data,function(indx,item){
				if(arr.indexOf(item.id)<0){
					domRow+='<input disabled="disabled"  name="villageIdList"  type="checkbox" value="'+item.id+'" >'+item.villageName+'</input>';
				}else{
					domRow+='<input disabled="disabled" name="villageIdList" checked="ture" type="checkbox" value="'+item.id+'" >'+item.villageName+'</input>';
				}
			})
