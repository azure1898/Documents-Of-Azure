...
public class SysCodeMaxService extends CrudService<SysCodeMaxDao, SysCodeMax> {
	 ...
	 * 获取订单编号
	 * @param cityId 城市ID
	 * @param businessInfoId 商家ID
	 * @return
	 */
	@Transactional(readOnly = false)
	public String getFieldOrdNo(String cityId,String businessInfoId){
		Date date=new Date();
		String year=DateUtils.getYear();
		String month=DateUtils.getMonth();
		String day=DateUtils.getDay();
		String no=String.format("%04d", this.getCodeNo( "001", 1, "true"));
		String moduleManageId=dao.getModuleManageId(businessInfoId);
		if (cityId==null){
			cityId="001";
		}
		if (moduleManageId==null){
			moduleManageId="01";
		}
		return cityId+moduleManageId+year.substring(2,4)+month+day+no;
	}
