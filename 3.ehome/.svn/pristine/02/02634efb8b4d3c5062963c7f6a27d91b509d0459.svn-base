...
<body>
	<ul class="nav nav-tabs">
		<li><span><a href="${ctx}/service/serviceInfo/">服务管理</a> > <a href="${ctx}/service/serviceInfo/form?id=${serviceInfo.id}">${not empty serviceInfo.id?'编辑':'新增'}服务</a></span></li>
	</ul>
	<br />
...
<div class="img-box full" id="imgArea">
		...
		<div class="control-group"	>
			...
			<div class="controls">
				<form:input path="basePrice" placeholder="请填写服务原价" maxlength="10"
					htmlEscape="false" class="input-medium required" value="${fns:doubleFormatForInput(serviceInfo.basePrice)}"/>
				<span class="help-inline"><font color="red">*</font> </span> 
				<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
				<span>优惠价：</span>
				<form:input path="benefitPrice" htmlEscape="false" placeholder="请填写优惠价格" maxlength="10"
					class="input-medium"  value="${fns:doubleFormatForInput(serviceInfo.benefitPrice)}"/>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
				<span>库存：</span>
		...
		<div class="form-actions">
            ...
            <c:choose>
				<c:when test="${not empty requestSrc and requestSrc=='sys_index'}">
					<input id="btnCancel" class="commonbtn" type="button" onclick="returnToSysIndex()" value="取消" style="width:60px"/>
				</c:when>
				<c:otherwise>
            		<input id="btnCancel" class="commonbtn" type="button" onclick="window.location.href='${ctx}/service/serviceInfo/list?sortItem=${serviceInfo.sortItem}&sort=${serviceInfo.sort}'" value="取消" style="width:60px"/>
				</c:otherwise>
			</c:choose>
		</div>
	</form:form>
