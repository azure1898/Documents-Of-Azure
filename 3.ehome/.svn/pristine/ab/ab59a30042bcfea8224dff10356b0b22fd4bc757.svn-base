...
public class MyFDFSClientUtils {
	 ...
	 * @throws IOException
	 */
	public static String uploadFile(String file_base64, String file_ext_name, HttpServletRequest request) throws IOException, MyException {
	 ...
	 * @throws IOException
	 */
	public static String uploadFile(MultipartFile file, HttpServletRequest request, boolean copassFlg) throws IOException, MyException {
	 ...
	 * @throws IOException
	 */
	public static String uploadFile(String file_base64, String file_ext_name, boolean copassFlg, HttpServletRequest request) throws IOException, MyException {
		...
		if (copassFlg) {
			...
			if ((result = myFDFSClient.upload_file(my_file_id, file1.getAbsolutePath(), file_ext_name.toUpperCase())) != 0) {
				System.err.println("upload_file fail, errno: " + result);
				return "";
			}
			File file2 = resize(220, 165, ret, file_ext_name);
			if ((result = myFDFSClient.upload_file(my_file_id + "_compress1", file2.getAbsolutePath(), file_ext_name.toUpperCase())) != 0) {
				System.err.println("upload_file fail, errno: " + result);
				return "";
			}
			File file3 = resize(134, 100, ret, file_ext_name);
			if ((result = myFDFSClient.upload_file(my_file_id + "_compress2", file3.getAbsolutePath(), file_ext_name.toUpperCase())) != 0) {
				System.err.println("upload_file fail, errno: " + result);
				return "";
			}
			ret.deleteOnExit();
	 ...
	 * @throws MyException
	 */
	public static String get_fdfs_file_id(HttpServletRequest request, String my_file_id) throws FileNotFoundException, IOException, MyException {
		String ctxPath = RequestContextUtils.getWebApplicationContext(request).getServletContext().getRealPath("/");
		MyFastDFSClient.init(ctxPath + fdfsConfigFilename, ctxPath + fdhtConfigFilename);
		MyFastDFSClient myFDFSClient = new MyFastDFSClient(fdhtNamespace);
	 ...
	 * @throws MyException
	 */
	public static int delete_file(HttpServletRequest request, String my_file_id) throws FileNotFoundException, IOException, MyException {
		String ctxPath = RequestContextUtils.getWebApplicationContext(request).getServletContext().getRealPath("/");
		MyFastDFSClient.init(ctxPath + fdfsConfigFilename, ctxPath + fdhtConfigFilename);
		MyFastDFSClient myFDFSClient = new MyFastDFSClient(fdhtNamespace);
