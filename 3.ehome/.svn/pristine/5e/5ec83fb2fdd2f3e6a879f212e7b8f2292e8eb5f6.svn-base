	...
	<div class="head-border">
		...
		<ul class="ul">
			<shiro:hasPermission name="goods:goodsInfo:view">
				<li><span>商&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;品：</span>${goodsCount}（库存不足：<a onclick="openedit('商品管理','/goods/goodsInfo')">${goodsCountStock}</a>）</li>
			</shiro:hasPermission>
		 	<shiro:hasPermission name="service:serviceInfo:view">
				<li><span>服&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;务：</span>${serviceCount}（库存不足：<a onclick="openedit('服务管理','/service/serviceInfo')">${serviceCountStock}</a>）</li>
			</shiro:hasPermission>
			<shiro:hasPermission name="lesson:lessonInfo:view">
				<li><span>课程培训：</span>${lessonCount}（约满：<a onclick="openedit('课程培训管理','/lesson/lessonInfo')">${lessonCountStock}</a>）</li>
			</shiro:hasPermission>
			<shiro:hasPermission name="field:fieldInfo:view">
				<li><span>场地预约：</span>${fieldCount}（约满：<a onclick="openedit('场地预约管理','/field/fieldInfo')">${fieldCountStock}</a>）</li>
			</shiro:hasPermission>
		</ul>
	</div>
	<div class="head-border">
		...
		<ul class="ul">
			<shiro:hasPermission name="goods:goodsInfo:view">
				<li><span>商品订单：</span><a onclick="openedit('商品订单','/order/orderGoods')">${goodsOrderCount}</a></li>
			</shiro:hasPermission>
		 	<shiro:hasPermission name="service:serviceInfo:view">
				<li><span>服务订单：</span><a onclick="openedit('服务订单','/order/orderService')">${serviceOrderCount}</a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="lesson:lessonInfo:view">
				<li><span>课程培训：</span><a onclick="openedit('课程培训订单','/order/orderLessonInfo')">${lessonOrderCount}</a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="field:fieldInfo:view">
				<li><span>场地预约：</span><a onclick="openedit('场地预约订单','/order/fieldInfo')">${fieldOrderCount}</a></li>
			</shiro:hasPermission>
		</ul>
	</div>
	<div class="head-border">
		...
		<ul class="ul">
			<shiro:hasPermission name="goods:goodsInfo:view">
				<li><span>商品订单：</span>￥${goodsMoney}</li>
			</shiro:hasPermission>
		 	<shiro:hasPermission name="service:serviceInfo:view">
				<li><span>服务订单：</span>￥${serviceMoney}</li>
			</shiro:hasPermission>
			<shiro:hasPermission name="lesson:lessonInfo:view">
				<li><span>课程培训：</span>￥${lessonMoney}</li>
			</shiro:hasPermission>
			<shiro:hasPermission name="field:fieldInfo:view">
				<li><span>场地预约：</span>￥${fieldMoney}</li>
			</shiro:hasPermission>
		</ul>
	</div>
</div>
...
<shiro:hasPermission name="goods:goodsInfo:view">
    <ul class="nav nav-tabs" style="width: 94%;border-bottom-style: none;margin-top: 10px;">
        ...
        <li style="width: 100%;">
            <div style="float: left;width: 120px;">待付款商品订单：<a href="#" onclick="openedit('商品订单','/order/orderGoods?payState=0')" style="color: #60C1F7;">${goodsSum1}</a></div>
            <div style="float: left;width: 120px;">待受理商品订单：<a href="#" onclick="openedit('商品订单','/order/orderGoods?orderState=0')" style="color: #60C1F7;">${goodsSum2}</a></div>
            <div style="float: left;width: 120px;">待配送商品订单：<a href="#" onclick="openedit('商品订单','/order/orderGoods?orderState=1')" style="color: #60C1F7;">${goodsSum3}</a></div>
            <div style="float: left;width: 120px;">待完成商品订单：<a href="#" onclick="openedit('商品订单','/order/orderGoods?orderState=2')" style="color: #60C1F7;">${goodsSum4}</a></div>
        </li>
    </ul>
...
<ul class="nav nav-tabs" style="width: 94%;border-bottom-style: none;margin-top: 10px;">
    ...
    <li style="width: 100%;">
        <div style="float: left;width: 120px;">待付款服务订单：<a href="" onclick="openedit('服务订单','/order/orderService?payState=0')" style="color: #60C1F7;">${serviceSum1}</a></div>
        <div style="float: left;width: 120px;">待受理服务订单：<a href="" onclick="openedit('服务订单','/order/orderService?orderState=0')" style="color: #60C1F7;">${serviceSum2}</a></div>
        <div style="float: left;width: 120px">待完成服务订单：<a href="" onclick="openedit('服务订单','/order/orderService?orderState=1')" style="color: #60C1F7;">${serviceSum3}</a></div>
    </li>
</ul>
<table id="contentTable" class="table table-striped table-bordered table-condensed" style="width: 99%;">
    <thead>
    <tr>
        ...
        <th>订单状态</th>
        <th>时间</th>
        <shiro:hasPermission name="order:orderService:edit"><th  width="100px;">操作</th></shiro:hasPermission>
    </tr>
    </thead>
    <tbody>
    <c:forEach items="${pageService.list}" var="orderService" varStatus="status">
        <tr>
            ...
            <td><shiro:hasPermission name="order:orderService:view">
                <input id="btuElemView" type="button" class="commonsmallbtn" value="查看" style="width:40px;margin-top: 2px;" onclick="window.location.href='${ctx}/order/orderService/form?id=${orderService.id}'">
                ...
                <shiro:hasPermission name="order:orderService:accept">
                    <c:if test="${orderService.payState == '1' && orderService.orderState ==  '0'}">
                        <input id="btuElemAccept" type="button" class="commonsmallbtn" value="受理" style="width:40px;margin-top: 2px;" onclick="window.location.href='${ctx}/order/orderService/accept?orderNo=${orderService.orderNo}&updateDate=${orderService.updateDateString}'">
                        <br/>
                    </c:if>
                </shiro:hasPermission>
                <shiro:hasPermission name="order:orderService:complete">
                    <c:if test="${orderService.payState == '1' && orderService.orderState ==  '1'}">
                        <input id="btuElemComplete" type="button" class="commonsmallbtn" value="完成" style="width:40px;margin-top: 2px;" onclick="window.location.href='${ctx}/order/orderService/complete?orderNo=${orderService.orderNo}&updateDate=${orderService.updateDateString}'">
                        <br/>
                    </c:if>
                </shiro:hasPermission>
...
<c:if test="${businessInfo.stockWarn=='1'}">
    ...
    <table id="contentTable" class="table table-bordered table-condensed" style="width: 99%;">
        <thead>
        <tr>
            <th >编号
            </th>
