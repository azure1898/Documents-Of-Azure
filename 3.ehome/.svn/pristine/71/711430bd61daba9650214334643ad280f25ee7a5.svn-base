...
<body>
	<form:form id="searchForm" modelAttribute="serviceInfo" action="${ctx}/service/serviceInfo/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
            ...
            <input id="btnAddService" class="commonbtn" style="width:80px" type="button" onclick="window.location.href='${ctx}/service/serviceInfo/form?sortItem=${serviceInfo.sortItem}&sort=${serviceInfo.sort}'" value="增加服务"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed" style="width:98%">
		...
		<c:forEach items="${page.list}" var="serviceInfoTemp" varStatus="status">
           ...
           <c:forEach items="${serviceInfoTemp.imageUrls}" var="imgUrl">
				...
				<td>
					<input type="button" class="commonsmallbtn" value="编辑" style="width:40px;" onclick="window.location.href='${ctx}/service/serviceInfo/form?id=${serviceInfoTemp.id}&sortItem=${serviceInfo.sortItem}&sort=${serviceInfo.sort}'">
				    <div style="margin:0 auto;height:3px;"></div>
				    <input type="button" class="commonsmallbtn" value="删除" style="width:40px;" onclick="return confirmx('确定删除此服务？删除后该服务的相应已受理订单依然需要完成。', '${ctx}/service/serviceInfo/delete?id=${serviceInfoTemp.id}&sortItem=${serviceInfo.sortItem}&sort=${serviceInfo.sort}')">
				</td>
			</tr>
		</c:forEach>
		</tbody>
