...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			if ($("#registDateEnd").val() < $("#registDateStart").val()) {
...
<body>
	...
	<form:form id="searchForm" modelAttribute="roomCertify" action="${ctx}/account/roomCertify/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			 <li>
				<input id="registDateStart" name="registDateStart" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${roomCertify.registDateStart}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
				-
				<input id="registDateEnd" name="registDateEnd" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${roomCertify.registDateEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
			<li class="btns">
