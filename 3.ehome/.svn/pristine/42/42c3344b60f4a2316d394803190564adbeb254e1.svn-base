	...
	public String personalInfo(String subUserId, String userId, int pageIndex) {
		...
		if(ssbList != null && ssbList.size()>0) {
			for(SocialSpeakBean ssb : ssbList) {
				...
				dataListMap.put("id", ssb.getId());
				dataListMap.put("createTime", ssb.getCreatetime());
				dataListMap.put("speakContent", ssb.getContent());
				...
				if(ssb.getIsspeak() == SocialGlobal.SPEAK_IS_SPEAK_YES) {//如果该发言是用户直接发言
					dataListMap.put("isForword", SocialGlobal.SPEAK_IS_FORWORD_NO);//是否转发为否
				} else {
					dataListMap.put("isForword", SocialGlobal.SPEAK_IS_FORWORD_YES);//是否转发为是
					//查询转发的发言
