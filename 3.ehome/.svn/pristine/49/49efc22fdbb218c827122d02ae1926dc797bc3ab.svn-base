        ...
        <c:forEach items="${page.list}" var="orderGoods" varStatus="status">
            <tr>
                ...
                <shiro:hasPermission name="order:orderGoods:edit"><td>
                    <a id="btuElemView" class="btn btn-primary" href="${ctx}/order/orderGoods/form?id=${orderGoods.id}"><i class="icon-align-justify icon-custom"></i>查看</a><br/>
                    <c:if test="${orderGoods.payState == '1' && orderGoods.orderState ==  '0'}">
                        <a id="btuElemAccept" class="btn btn-primary" href="${ctx}/order/orderGoods/accept?id=${orderGoods.id}&updateDate=${orderGoods.updateDateString}"><i class="icon-ok icon-custom"></i>受理</a><br/>
                    </c:if>
                    <c:if test="${orderGoods.payState == '1' && orderGoods.orderState ==  '2'}">
                        <a id="btuElemComplete" class="btn btn-primary" href="${ctx}/order/orderGoods/complete?id=${orderGoods.id}&updateDate=${orderGoods.updateDateString}"><i class="icon-ok-circle icon-custom"></i>完成</a><br/>
                    </c:if>
