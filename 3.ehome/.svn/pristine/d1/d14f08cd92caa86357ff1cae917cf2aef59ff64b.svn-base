...
<script type="text/javascript">
    ...
    function bindPosition() {
        console.log($("#villageLineId").val())
        var product_line = $("#villageLineId").val();
        $.ajax({
            ...
            success : function(data) {
                var hidPositionId = $("#hidPositionId").val();
                $("#positionId").empty();
                var option = "<option value=''>广告位置</option>";
                $.each(data, function(indx, item) {
                    option += "<option value='"+item.id+"'>" + item.positionName + "</option>";
                })
                $("#positionId").append(option);
                $("#positionId").val(hidPositionId).trigger("change");//修改初始时，带值选中
            }
        })
    }
</script>
...
<body>
    <ul class="nav nav-tabs">
        ...
        <shiro:hasPermission name="adver:adverManage:edit">
            <li>
                <a href="${ctx}/adver/adverManage/form">广告信息添加</a>
            </li>
        </shiro:hasPermission>
    </ul>
    <form:form id="searchForm" modelAttribute="adverManage" action="${ctx}/adver/adverManage/" method="post" class="breadcrumb form-search">
        ...
        <ul class="ul-form">
            ...
            <li class="btns">
                <form:select path="villageLineId" class="input-medium" onchange="bindPosition()">
    ...
    <ul style="margin: 10px;">
        <li>
            ...
            <shiro:hasPermission name="adver:adverManage:edit">
                <a id="btuElemEdit" href="${ctx}/adver/adverManage/form?id=" class="btn btn-primary" onclick="return elemEdit('广告')"><i class="icon-edit icon-custom"></i>编辑</a>
            </shiro:hasPermission>
            <shiro:hasPermission name="adver:adverManage:delete">
                <a id="btuElemDelete" href="${ctx}/adver/adverManage/delete?id=" class="btn btn-primary" onclick="return elemDelete('广告')"><i class="icon-trash icon-custom"></i>删除</a>
            </shiro:hasPermission>
        </li>
    </ul>
        ...
        <tbody>
            <c:forEach items="${page.list}" var="adverManage" varStatus="status">
                <tr onClick="selectElem(this)">
                    ...
                    <td>${adverManage.adverPosition.positionName}</td>
                    <td>${fn:substringBefore(adverManage.villageInfo.addrPro, ",")}</td>
                    <td>${fn:substringBefore(adverManage.villageInfo.addrCity, ",")}</td>
