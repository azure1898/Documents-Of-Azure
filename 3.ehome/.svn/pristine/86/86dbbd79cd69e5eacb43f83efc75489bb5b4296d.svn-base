...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sys/role/form?id=${role.id}">角色<shiro:hasPermission name="sys:role:edit">授权</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
		...
		<div class="control-group" style="display: none;">
			...
			<div class="controls">
                <sys:treeselect id="office" name="office.id" value="1" labelName="office.name" labelValue="${role.office.name}"
		...
		<div class="control-group" style="display: none;">
			...
			<div class="controls">
				...
				<form:input path="enname" value="abc" htmlEscape="false" maxlength="50" class="required"/>
		...
		<div class="control-group" style="display: none;">
			...
			<div class="controls">
				<form:select path="roleType" class="input-medium">
					<form:option value="assignment">普及内部</form:option>
					<form:option value="security-role">实地物业</form:option>
				</form:select>
				<span class="help-inline" title="activiti有3种预定义的组类型：security-role、assignment、user 如果使用Activiti Explorer，需要security-role才能看到manage页签，需要assignment才能claim任务">
					工作流组用户组类型（普及内部：assignment、实地物业：security-role、普通角色：user）</span>
			</div>
		</div>
		<div class="control-group" style="display: none;">
		...
		<div class="form-actions">
			<c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
				<shiro:hasPermission name="sys:role:edit"><input id="btnSubmit" class="btn btn-success" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
			</c:if>
