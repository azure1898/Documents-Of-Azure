function dealData() {
    var vm = new Vue({
            ...
            this.$nextTick(function () { //在2.0版本中，加mounted的$nextTick方法，才能使用vm
                ...
                this.getData(_this, "/live/getSites", data, function (resData) {
                    ...
                    _this.getData(_this, "/live/getSiteReservation", iteReservationData, function (resData2) {
                        ...
                        _this.init(_this.place.isNormal, _this.siteReservation.length, _this.fullAppointment);
        ...
        methods: {
            init: function (status, timeLength, bookLength) {
                console.log(status, timeLength, bookLength);
                if (status == 1) {
                    if (timeLength > 0) {
                        if (timeLength > bookLength) {
                            this.isOK = true;
                            $("#appointment").removeClass().addClass("ser_zbtn");
                            $("#appointment >a").text("立即预约")
                        }
                        else if (timeLength = bookLength) {
                            this.isOK = false;
                            $("#appointment").removeClass().addClass("ser_zbtn2");
                            $("#appointment >a").text("预约已满");
                        }
                    }
                    else {
                        this.isOK = false;
